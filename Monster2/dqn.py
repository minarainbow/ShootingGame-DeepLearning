...
GAME = 'monster' # the name of the game being played for log files
...
def trainNetwork(s, readout, sess):
    ...
    action = tf.reduce_sum(tf.multiply(readout, a), reduction_indices=1)
    # cost function we will reduce through backpropagation
    cost = tf.reduce_mean(tf.square(action - action))
    ...
    while "flappy bird" != "angry bird":
        ...
        if t % 10000 == 0:
            saver.save(sess, 'saved/' + GAME + '-dqn', global_step = t)
